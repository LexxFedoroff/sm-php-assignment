    ...
    public function getTestCases(): array
    {
        return [
            'empty posts => empty statistic'          =>
                $this->generateTestCase(d('2022-03-01'), d('2022-03-31'), [], stat([])),
            'there are no posts in March'             =>
                $this->generateTestCase(d('2022-03-01'), d('2022-03-31'), $this->generatePosts(), stat([])),
            'there are 3 authors in January'  =>
                $this->generateTestCase(d('2022-01-01'), d('2022-01-31'), $this->generatePosts(), stat([
                    'Author 1' => 2,
                    'Author 2' => 1,
                    'Author 3' => 3,
                ])),
            'there are 1 author in February' =>
                $this->generateTestCase(d('2022-02-01'), d('2022-02-28'), $this->generatePosts(), stat([
                    'Author 2' => 2,
                ])),
            'there are 2 authors in from January,20 to February,20' =>
                $this->generateTestCase(d('2022-01-20'), d('2022-02-20'), $this->generatePosts(), stat([
                    'Author 3' => 2,
                    'Author 2' => 2,
                ]))
        ];
